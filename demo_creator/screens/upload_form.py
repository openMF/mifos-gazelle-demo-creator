...
class UploadScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="upload_form"):
            ...
            self.repo_url = Input(placeholder="e.g., https://<org>.jfrog.io/artifactory/<repo>/")
            yield self.repo_url
    ...
    def sync_deleted_and_upload(self):
        ...
        def on_delete_done(success_delete, msg_delete):
            ...
            self.app.call_from_thread(
                self.status.update, f"[yellow]Uploading files... Please wait.\n"
                + (msg_delete or "")
            )
            def on_upload_complete(success, msg_upload):
                ...
                self.app.call_from_thread(
                    self.status.update, f"[{color}]{msg_delete or ''}\n{msg_upload or ''}"
                )
                self.app.call_from_thread(
